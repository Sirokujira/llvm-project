//===----- RXMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define DEBUG_TYPE "rx-merge-base-offset"
#define RX_MERGE_BASE_OFFSET_NAME "RX Merge Base Offset"
namespace {

struct RXMergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  RXMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return RX_MERGE_BASE_OFFSET_NAME;
  }
...
char RXMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(RXMergeBaseOffsetOpt, "rx-merge-base-offset",
                RX_MERGE_BASE_OFFSET_NAME, false, false)
...
bool RXMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  return true;
}
...
// output from LoADDI.
void RXMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool RXMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               unsigned GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == RX::ADD) && "Expected ADD instruction!");
  ...
  /*
  if (OffsetTail.getOpcode() == RX::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != RXII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != RX::LUI ||
        LuiImmOp.getTargetFlags() != RXII::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == RX::LUI) {
...
bool RXMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool RXMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createRXMergeBaseOffsetOptPass() {
  return new RXMergeBaseOffsetOpt();
}
