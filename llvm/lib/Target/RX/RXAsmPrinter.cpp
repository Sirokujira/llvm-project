...
public:
  ...
  bool lowerOperand(const MachineOperand &MO, MCOperand &MCOp) const {
    return LowerRXMachineOperandToMCOperand(MO, MCOp, *this);
  }
};
}
...
#include "RXGenMCPseudoLowering.inc"
...
bool RXAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
                                      unsigned AsmVariant,
  ...
  if (!ExtraCode) {
    ...
    case MachineOperand::MO_Register:
      OS << RXInstPrinter::getRegisterName(MO.getReg());
      return false;
    default:
      break;
    }
...
bool RXAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
  ...
  if (!ExtraCode) {
    ...
    OS << "0(" << RXInstPrinter::getRegisterName(MO.getReg()) << ")";
    return false;
  }
...
extern "C" void LLVMInitializeRXAsmPrinter() {
  RegisterAsmPrinter<RXAsmPrinter> X(getTheRXTarget());
}
